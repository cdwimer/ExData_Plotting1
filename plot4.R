filename <- "exdata_data_household_power_consumption.zip"
if(!file.exists(filename)){
    fileurl <- "https://d396qusza40orc.cloudfront.net/exdata%2Fdata%2Fhousehold_power_consumption.zip"
    download.file(fileurl, "exdata_data_household_power_consumption.zip")
}

if(!file.exists("household_power_consumption.txt")){
    unzip(filename)
}

hpc<-read.table(file.path(getwd(), "household_power_consumption.txt"), header = TRUE, sep = ";", stringsAsFactors = FALSE, dec = ".")
subsetHPC <- hpc[hpc$Date %in% c("1/2/2007","2/2/2007"),]
weekday <- strptime(paste(subsetHPC$Date, subsetHPC$Time, sep = " "), "%d/%m/%Y %H:%M:%S")

png("plot4.png", width = 480, height = 480)
par(mfrow = c(2, 2)) 

plot(weekday, as.numeric(subsetHPC$Global_active_power), type = "l", xlab = "", ylab = "Global Active Power (kilowatts)")

plot(weekday, as.numeric(subsetHPC$Voltage), type = "l", xlab = "datetime", ylab = "Voltage")

plot(weekday, as.numeric(subsetHPC$Sub_metering_1), type = "l", xlab = "", ylab = "Energy sub metering")
lines(weekday, as.numeric(subsetHPC$Sub_metering_2), type = "l", col = "red")
lines(weekday, as.numeric(subsetHPC$Sub_metering_3), type = "l", col = "blue")
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty = 1, lwd = 2, col = c("black", "red", "blue"), bty = "n")

plot(weekday, as.numeric(subsetHPC$Global_reactive_power), type = "l", xlab = "datetime", ylab = "Global_reactive_power")

dev.off()